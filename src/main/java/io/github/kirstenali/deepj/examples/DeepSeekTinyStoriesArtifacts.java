...
final class DeepSeekTinyStoriesArtifacts {
    ...
    static Loaded load(Path output, Path checkpoint) throws IOException {
        ...
        DeepSeekConfig config = config(properties, tokenizer.vocabSize());
        DeepSeekModel model = new DeepSeekModel(config, longValue(properties, "seed"));
    ...
    private static DeepSeekConfig config(Properties properties, int vocabSize) {
        return new DeepSeekConfig(vocabSize, intValue(properties, "sequenceLength"),
    ...
    record Loaded(Properties properties, BPETokenizer tokenizer,
                  DeepSeekConfig config, DeepSeekModel model) {}
}
